...
\begin{enumerate}[wide, labelwidth=!, labelindent=0pt]\vs \item The Prover $\mP$ chooses random $k, e_1\in[-B,B]$ and sends $A_1:= g^{k}$ to the Verifier $\V$.

\noin 2. The hashing algorithm $\ttt{H}_{\ttt{FS},\lam}$ generates a $\lam$-bit prime $\gamma$.

\noin 3. $\mP$ chooses a random $e_2 \in [-B,B]$ and sends $A_2:= g^{f(d_1+k\gamma)}h^{e_2}$ to $\V$.

\noin 4. The hashing algorithm $\ttt{H}_{\ttt{FS},\lam}$ generates a $\lam$-bit prime $\ell$. 

\noin 5. $\mP$ computes the integers $q_1$, $r_1$, $q_2$, $r_2$, $q_2'$, $s_2$ such that \vs $$d_1+k\gamma = q_1\cdot \ell + r_1\;,\; \mbf{f}(d_1+k\gamma) = q_2\cdot \ell + r_2 \;,\;\;e_2 = q_2'\cdot \ell+s_2  \;\;,\;\;r_1,r_2,s_2\in [\ell] \vs $$ and sends $r_1$, $s_2$, $Q_1:= g^{q_1}h^{q_1'}\;,\; Q_2:= g^{q_2}h^{q_2'}$ to $\V$.

\noin 6. $\mP$ computes $\wti{h}:= h^{e-e_2}$ and sends it to $\V$ along with a non-interactive proof for $\ttt{ZKPoKE}[h,\;\wti{h}]$

\noin 7. $\mP$ generates a non-interactive proof for $\ttt{ZKPoKE}[A_1^{\gamma},\;A_2\cdot b^{-1}\cdot \wti{h}]$.

\noin 8. $\V$ verifies the $\ttt{ZKPoKE}$s he receives. 

\noin 9. $\V$ verifies that $r_1,s_2\in [\ell]$ and computes $r_2:= \mbf{f}(r_1)\Mod{\ell}$. He verifies the equations \vs $$Q_1^{\ell}g^{r_1}h^{s_1} \sta a\cdot A_1^{\gamma}\;\;\bigwedge\;\; Q_2^{\ell}g^{r_2}h^{s_2}\sta A_2 \vs $$ and accepts if and only if all equations hold and all of the $\tt{ZKPoKE}$s are valid.\qed \end{enumerate}
