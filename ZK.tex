...
\noin 5. $\V$ verifies the three $\tt{ZKPoKE}$s and the one $\ttt{ZKPoEqDLog}$ he receives and the equation \vs $$ \what{a}_2\cdot \wti{h}_2 \sta a_2\;\;\bigwedge\;\;\what{a}_3\cdot \wti{h}_3 \sta a_3. \vs $$ He accepts if and only if both equations hold and all three $\tt{ZKPoKE}$s and the $\ttt{ZKPoEqDLog}$ are valid.\qed \end{enumerate}
